...
class Casco:
    ...
    def plotar_casco_3d(self) -> str:
        ...
        # # 1. Adiciona as seções transversais (Balizas)
        # for x_val in self.posicoes_balizas:
        #     df_baliza = self.df[self.df['X'] == x_val].sort_values('Z')
        #     # Combina os pontos do lado direito e esquerdo para um único traço
        #     y_completo = pd.concat([df_baliza['Y'], -df_baliza['Y'][::-1]])
        #     z_completo = pd.concat([df_baliza['Z'], df_baliza['Z'][::-1]])
        #     x_completo = [x_val] * len(y_completo)
            
        #     fig.add_trace(go.Scatter3d(
        #         x=x_completo, y=y_completo, z=z_completo,
        #         mode='lines',
        #         line=dict(color='cyan', width=2),
        #         name=f'Baliza X={x_val:.2f}'
        #     ))
        
        # # 2. Adiciona o perfil da quilha
        # if self.funcao_perfil:
        #     x_interp = np.linspace(min(self.posicoes_balizas), max(self.posicoes_balizas), 50)
        #     z_interp = self.funcao_perfil(x_interp)
        #     fig.add_trace(go.Scatter3d(
        ...
        # # 3. Configura o layout da cena 3D
        # fig.update_layout(
        #     title_text='Visualização 3D do Casco',
        #     scene=dict(
        #         xaxis_title='Comprimento (X)',
        ...
        #         aspectmode='data' # Tenta manter as proporções realistas
        #     ),
        #     margin=dict(l=0, r=0, b=0, t=40)
        # )
        # print("-> Gráfico 3D gerado. Convertendo para HTML.")
        # return fig.to_html(full_html=False, include_plotlyjs=False)
